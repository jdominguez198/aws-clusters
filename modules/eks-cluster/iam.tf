...
module "iam_iam-assumable-role-with-oidc" {
  ...
  role_name             = "${var.name_prefix}-${local.openid_connect_role_name}"
  create_role           = true
...
resource "aws_iam_policy" "load-balancer-policy" {
  name = "${var.name_prefix}-${local.load_balancer_role_name}"
  path = "/"
  policy = file("${path.module}/iam/iam-load-balancer-policy.json")
}

resource "aws_iam_role_policy_attachment" "AmazonLoadBalancer" {
  policy_arn = "arn:aws:iam::${data.aws_caller_identity.current.account_id}:policy/${var.name_prefix}-${local.load_balancer_role_name}"
...
resource "aws_iam_policy" "cluster_autoscaler" {
  count = 1
  name = "${var.name_prefix}-${local.cluster_autoscaler_role_name}"
  path = "/"
  policy = file("${path.module}/iam/iam-autoscaler-policy.json")
}

resource "aws_iam_role_policy_attachment" "cluster_autoscaler" {
  count = 1

  role = "${var.name_prefix}-${local.openid_connect_role_name}"
